...
int get_valid_part_num(block_dev_desc_t *dev_desc)
{
    ulong i, n;
    if (check_partition_table_version(dev_desc, &g_ddesc) < 0)
        return;
...
int get_single_partition_info(uchar index, disk_partition_t *disk_partition)
{
    if (g_mpart[index].signature != EMMC_PARTITION_MAGIC) 
    {
        printf("error EMMC_PARTITION_MAGIC %x\n", g_mpart[index].signature);
        return 1;
    }

    disk_partition->size = g_mpart[index].block_count*512;
    disk_partition->start = g_mpart[index].start_block;
    memcpy(disk_partition->name, g_mpart[index].name, sizeof(disk_partition->name));
    memcpy(disk_partition->type, g_mpart[index].type, sizeof(disk_partition->type));
...
int rmgpt_emmc_partition(int curr_device, char *symbol)
{
    ...
    if (force_rmgpt == 0)
    {
        #ifdef CONFIG_MBOOT_IN_MMC_FLASH
...
////#else
//    memset(&dpt, 0, sizeof(dpt));
//    Configure_Boot(mmc_dev, &dpt, 1);   // 1 --> MBOOT
...
//    }
//    memset(&dpt, 0, sizeof(dpt));
//    Configure_Boot(mmc_dev, &dpt, 0);   // 0 --> MPOOL
//    if (add_new_emmc_partition(mmc_dev, &dpt) != 0)
//    {
//        printf("Add new partition MPOOL failed!\n");
//        return -1;
//    }
//
//    #ifdef CONFIG_DOUBLE_MBOOT
...
//    {
//        printf("Configure_MBOOTBAK failed!\n");
//        return -1;
//    }
//    if (add_new_emmc_partition(mmc_dev, &dpt) != 0)
//    {
//        printf("Add new partition MBOOTBAK failed!\n");
//        return -1;
//    }
//    #endif
//    printf("add_ca_partition_emmc(Mstar) success !\n");
////#endif
//    return 0;
//}
